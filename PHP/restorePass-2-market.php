...
$conn = dbConnect();
$sql = "SELECT * FROM `passwordSessionMarket` WHERE `key` = '".$key."' AND `timestamp` = CURDATE()";
...
$sql = "UPDATE `marketLogins` SET `password` = :password WHERE `email` = :mail";
$sql = $dbh->prepare($sql);
...
$sql = "DELETE FROM `passwordSessionMarket` WHERE `user` = '".$mail."'";
$conn->query($sql);
